...
func load_state() -> Dictionary:
	if not FileAccess.file_exists(journal_path):
		return {"drafts": [], "camera_profiles": {}, "lighting": {}}
	var file := FileAccess.open(journal_path, FileAccess.READ)
	if not file:
		return {"drafts": [], "camera_profiles": {}, "lighting": {}}
	var parsed: Variant = JSON.parse_string(file.get_as_text())
	return parsed if parsed is Dictionary else {"drafts": [], "camera_profiles": {}, "lighting": {}}


func save_state(drafts: Array, camera_profiles: Dictionary, lighting: Dictionary = {}) -> bool:
