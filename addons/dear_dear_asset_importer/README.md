...
3. Orbit with left-drag, pan with Shift-left or middle-drag, and zoom with the wheel. Camera profiles persist by category/subcategory. **Reset Camera Angle** resets only the active profile.
...
- Existing assets are scanned for ID use and collisions but are never renamed or backfilled. The current project intentionally reports the pre-existing duplicate ID `310148`.
