...
5. Make the intended row active and run **Capture & Save Market PNG Image**. The server permanently reserves its six-digit ID before the plugin copies the standardized GLB and saves its 1024×1024 transparent PNG.
6. Run **Export CSV / JSON** for the active captured record to upsert it into `data/asset_catalog.json` and regenerate `data/asset_exports/asset_catalog.csv`.
7. Run **Sync Google Sheets** for the active exported record to complete its reserved spreadsheet row. All actions are idempotent by `record_id`.
...
- Adding, reloading, or normally clicking a row makes only that row active. Ctrl/Shift multi-selection is used only by **Apply Category Settings to Selected**; **Capture**, **Export**, **Sync**, and **Remove Draft** always operate on the single active visible row. Superseded preview scenes are freed immediately, and the plugin lets Godot's filesystem watcher import newly written assets instead of forcing a second scan during an active import.
